...
# # Nits Consulta
# BANREP : NIT8600052167
# DAVIVIENDA : NIT8600343137
# ITAU : NIT8909039370


### BANREP ====
nit_consulta <- "CC19146113"
...
df_union <- bind_rows(df_afil2015,df_afil2016,df_afil2017,df_afil2018,df_afil2019) %>% 
  group_by(id_empresa,anio) %>% 
  summarise(conteo = sum(conteo)) %>% 
  data.frame() %>% 
  mutate(tasa = 100*(conteo - lag(conteo, n = 4))/lag(conteo, n = 4))
str(df_union)
...
# Consulta 
test <- bd_afiliados %>% 
  dplyr::filter(id_empresa %in% nit_consulta) %>% 
