...
shinyServer(function(input, output, session) {
  ...
  output$plot_pira_glob <- renderPlotly({
    ...
    aux1 %>%
      ...
      layout(margin = m,
             ...
             xaxis = list(title='Afiliados', titlefont = f1, tickfont = f2),
  ...
  output$pre_aprobado_hipotecario_glob <- renderValueBox({
    data_f1<-data_f1() %>% 
      select(id_persona,pre_aprobado_hipo)
    valueBox(
      value = paste(formatC(100*sum(data_f1$pre_aprobado_hipo, na.rm = T)/length(data_f1$id_persona),
  ...
  output$pre_aprobado_cupo_glob <- renderValueBox({
    data_f1<-data_f1() %>% 
      select(id_persona,pre_aprobado_cupo)
    valueBox(
      value = paste(formatC(100*sum(data_f1$pre_aprobado_cupo, na.rm = T)/length(data_f1$id_persona),
  ...
  output$conteo_cuad_a_glob <- renderValueBox({
    data_f1<-data_f1() %>% 
      select(id_persona,CuadranteViv)
    valueBox(
      value = paste(formatC(100*sum(data_f1$CuadranteViv == "A", na.rm = T)/length(data_f1$id_persona),
  ...
  output$conteo_cuad_a1_glob <- renderValueBox({
    data_f1<-data_f1() %>% 
      select(id_persona,CuadranteViv)
    valueBox(
      value = paste(formatC(100*sum(data_f1$CuadranteViv == "A1", na.rm = T)/length(data_f1$id_persona),
  ...
  output$conteo_cuad_a2_glob <- renderValueBox({
    data_f1<-data_f1() %>% 
      select(id_persona,CuadranteViv)
    valueBox(
      value = paste(formatC(100*sum(data_f1$CuadranteViv == "A2", na.rm = T)/length(data_f1$id_persona),
  ...
  output$conteo_cuad_b_glob <- renderValueBox({
    data_f1<-data_f1() %>% 
      select(id_persona,CuadranteViv)
    valueBox(
      value = paste(formatC(100*sum(data_f1$CuadranteViv == "B", na.rm = T)/length(data_f1$id_persona),
                            digits = 1, format = "f", big.mark=","),"%", " (",comma(sum(data_f1$CuadranteViv == "B", na.rm = T)),")",sep = ""),
