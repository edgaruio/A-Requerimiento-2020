...
dashboardPage(skin = "blue",
              dashboardHeader(title = "Información Consumos"),
              dashboardSidebar(
                ...
                tabItems(
                  tabItem("dummy",
                          fluidRow(
                            column(1),
                            column(10,
                                   h1("Aplicacion consumos Caja de Retiro de las Fuerzas Militares"),
                                   br(),br(),
                                   h3("La presente aplicacion tiene por objetivo ser una herramienta de consulta de consumos de afiliados"),
                                   br(),
                  ...
                  tabItem(tabName = "global",
                          fluidPage(
                            title = "DB",
                            fluidRow(
                              box(title = "Consolidado Información (Corte Diciembre)",width=12,status="primary",solidHeader=TRUE,collapsible=FALSE,collapsed = TRUE,
                                  fluidRow(
                            ...
                            fluidRow(
                              box(title = "Gestion de Afiliados Cobertura (Consolidado 2019)",width=12,status="primary",solidHeader=TRUE,collapsible=TRUE,collapsed = TRUE,
                                  fluidRow(
                                    valueBoxOutput("conteo_salud_glob",width = 2),
                                    valueBoxOutput("conteo_famisanar_glob",width = 2),
                                    valueBoxOutput("conteo_ips_glob",width = 2),
                                    valueBoxOutput("conteo_pac_famisanar_glob",width = 2),
                                    valueBoxOutput("conteo_sura_eps_glob",width = 2),
                                  ...
                                  fluidRow(
                                    ...
                                    # valueBoxOutput("conteo_ryt_glob",width = 2),
                            ...
                            fluidRow(
                              box(title = "Gestion de Derechos (Consolidado 2019)",width=12,status="primary",solidHeader=TRUE,collapsible=TRUE,collapsed = TRUE,
                                  fluidRow(
